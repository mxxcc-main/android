...
public class TabPageIndicator extends HorizontalScrollView {
    ...
    private int indicatorColor = Color.parseColor("#192737");//指示线的颜色  #0075E7
    private int underlineColor = R.color.color_0D8BCA;
    ;// 默认指示线的颜色
    ...
    private int tabTextSize = 14;//标题的字体大小
    private int tabTextColor = R.color.color_828EA1;// 标题未被选中时字体颜色
    private int tabTextColorSelected = R.color.color_1F3F59;// 标题被选中时字体颜色
...
//    int zfb = (R.mipmap.icon_up_unselected);
//    int dfb = (R.mipmap.icon_down_unselected);
//    int jyb = (R.mipmap.icon_trading_unselected);
//    int zfbs = (R.mipmap.icon_up_selected);
//    int dfbs = (R.mipmap.icon_down_selected);
//    int jybs = (R.mipmap.icon_trading_selected);
...
//        isSameLine = a.getBoolean(R.styleable.PagerSlidingTab_sameLine, isSameLine);
    ...
    public void setPosition(int position) {
        ...
        for (int i = 0; i < tabCount; i++) {
            ...
            if (v instanceof TextView) {
                ...
                if (i == position) {
                    tab_title.setTextColor(getResources().getColor(tabTextColorSelected));
                } else {

                    tab_title.setTextColor(getResources().getColor(tabTextColor));
                }
            }
        }
    }
    ...
    private void addTextTab(final int position, String title) {
        ...
        View view = LayoutInflater.from(MyApp.appContext).inflate(R.layout.tab_title, null);
        ...
        tab_title.setFocusable(true);
        tab_title.setGravity(Gravity.CENTER);
        tab_title.setSingleLine();
...
//        这里对首页的三个标签进行赛图标
        ...
        if (isExpand && !isExpandSameLine) {
            tab_title.setPadding(tabPadding, 0, tabPadding, 0);
        } else {
            wrapTabLayoutParams.setMargins(tabPadding, 0, tabPadding, 0);
            weightTabLayoutParams.setMargins(tabPadding, 0, tabPadding, 0);
        }

        tabsContainer.addView(view, position, isSameLine ? wrapTabLayoutParams : weightTabLayoutParams);
    }
    ...
    private void updateTabStyles() {
        ...
        for (int i = 0; i < tabCount; i++) {
            ...
            if (v instanceof TextView) {
                ...
                tab.setTextColor(i == 0 ? getResources().getColor(tabTextColorSelected) : getResources().getColor(tabTextColor));
                //大小写切换
                if (textAllCaps) {
        ...
        public void onPageSelected(int position) {
            ...
            for (int i = 0; i < tabCount; i++) {
                ...
                if (tab_title instanceof TextView) {
                    ...
                    tab_title.setTextColor(i == pager.getCurrentItem() ? getResources().getColor(tabTextColorSelected) : getResources().getColor(tabTextColor));
                }
            }
        }
    }
