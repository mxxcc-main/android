 ...
 */
public class UserRedemptionRecordsAty extends BaseAty implements OnRefreshListener, OnLoadMoreListener, HttpRequestCallback<Object> {
    ...
    private UserRedemptionRecordsAdapter userRedemptionRecordsAdapter;
    private int pageNo = 1;
    private int pageSize = 20;

    @Override
    public int initView() {
        return R.layout.user_redemption_records;
    }
...
//        StatusBarUtil.setStatusColor(this, true, false, R.color.color_2E303B);
        nllWalletMarket.setTitle("提现记录");

        dialogUtils.showProgressDialog();
        requestData(0);
        userRedemptionRecordsAdapter = new UserRedemptionRecordsAdapter(this, jsonArrayFinalRecord, (view, pojo, position) -> {
        });
        ...
        lRecyclerViewAdapter = new LRecyclerViewAdapter(userRedemptionRecordsAdapter);
        ...
        try {
            if (!TextUtils.isEmpty(result)) {
                if (type == 0) {
                    ...
                    userRedemptionRecordsAdapter.setqOrgs(jsonArrayFinalRecord);
                    userRedemptionRecordsAdapter.notifyDataSetChanged();
                    lr_id.refreshComplete(pageSize);
                }
